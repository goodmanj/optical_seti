...
# import csv
output = open("2800broadsearch.txt", "w")
output.write("STAR,SPECTRAL TYPE,START INDEX,END INDEX,FILE,START WAVELENGTH,END WAVELENGTH,HARPS OBJECT,TEMPERATURE,INTERMEDIATE_COUNT\n") #add wavelength variable.
...
for (star, harps_object, spectral_type, temperature, file, distance) in zip(target_list[1:], harps_objects[1:], spectral_types[1:], temperatures[1:], data_files[1:], distances[1:]): 
    ...
    hits_start, hits_end, count, intermediate_count = wormsearcher(arr1, min_count = 4, max_count = 60, threshold_multiplier = 3.5, stwindow = 101, window_size = 101)
    ...
    for (start, end) in zip(hits_start, hits_end):
            ...
            if len(hits_start) > 0 and len(hits_start) <= 3:
